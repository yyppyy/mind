...
int disaggr_open_file(const char __user *filename, int flags, umode_t mode ){
    open_file_req_t* request = kmalloc(sizeof(open_file_req_t));
    open_file_res_t* response = kmalloc(sizeof(open_file_res_t));
...
int disaggr_read_file(unsigned int fd, char* usr_buf, size_t num_bytes){
    read_file_req_t* request = kmalloc(sizeof(read_file_req_t));
    read_file_res_t* response = kmalloc(sizeof(read_file_res_t));
    request->fd = fd;
...
int disaggr_write_file(unsigned int fd, char* usr_buf, size_t num_bytes){
    write_file_req_t* request = kmalloc(sizeof(write_file_req_t));
    write_file_res_t* response = kmalloc(sizeof(write_file_res_t));
...
int disaggr_close_file(unsigned int fd){
    close_file_req_t* request = kmalloc(sizeof(close_file_req_t));
    close_file_res_t* response = kmalloc(sizeof(close_file_res_t));
    request.fd = fd;
